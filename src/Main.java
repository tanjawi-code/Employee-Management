...
public class Main {
    ...
    static void employeesInfo(EmployeeManager manager) {
        ...
        do {
            try {
                ...
                switch (choice) {
                    case 1:
                        manager.fixEmployeesDetails();
                        break;
                    case 2:
                        manager.hourlyEmployeeDetails();
                        break;
                    case 3:
                        manager.commissionEmployeeDetails();
                        break;
                    case 4:
                        manager.basePlusCommissionEmployees();
                        break;
                    case 5:
                        manager.allEmployeesDetails();
                        break;
                    default:
                        System.out.println("Wrong choice, try again.\n");
                }
    ...
    static void calculateSalaries(EmployeeManager manager) {
        ...
        do {
            try {
                ...
                switch (choice) {
                    case 1:
                        manager.employeesSalaries(employeeType.Fix);
                        break;
                    case 2:
                        manager.employeesSalaries(employeeType.Hourly);
                        break;
                    case 3:
                        manager.employeesSalaries(employeeType.Commission);
                        break;
                    case 4:
                        manager.employeesSalaries(employeeType.Base_Plus_Commission);
                        break;
                    default:
                        System.out.println("Wrong choice, try again.\n");
                }
    ...
    // A function for checking if the user wants to keep old data or remove it. It used only with (takeEmployeesFile).
    static void clearFile(int choice, String file) {
        if (choice == 1) {
    ...
    static void saveEmployeeFile(EmployeeManager manager) {
        String filePath = "C:\\Users\\asus\\desktop\\employeesData.csv";

        if (!manager.employeeIsEmpty()) {
            try (FileWriter file = new FileWriter(filePath)) {
                for (int x = 0; x < manager.employeesSize(); x++) {
                    file.write(manager.employeesFile(x));
                }
    ...
    // This is a method for bringing the employees from the file that the employees are saved in.
    static void takeEmployeesFile(EmployeeManager manager) {

        String FilePlace = "C:\\Users\\asus\\desktop\\employeesData.csv";
        String[] confirm = {"Yes, Use the old employees data", "No, Make new employees data"};
        int answer;
        File file = new File(FilePlace);

        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            answer = JOptionPane.showOptionDialog(null,
                    "Do you want to use the old data or making new data", "Employees data",
                    JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, confirm, 0);
            if (answer == 0) {
                if (!(file.length() == 0)) {
                    System.out.println("The file is exists. You have took the old data.\n");
                    String paragraph;
                    while ((paragraph = reader.readLine()) != null) {
                        ...
                        int SSN = Integer.parseInt(data[4]);
                        int ID = Integer.parseInt(data[5]);

                        switch (type) {
                            case Fix:
                                SalariedEmployee fix = new SalariedEmployee(name, SSN, address, gender, ID, type);
                                double salary = Double.parseDouble(data[6]);
                                double bonus = Double.parseDouble(data[7]);
                                double deduction = Double.parseDouble(data[8]);
                                fix.setSalary(salary);
                                fix.setBonus(bonus);
                                fix.setDeduction(deduction);
                                manager.saveEmployee(fix);
                                break;
                            case Hourly:
                                HourlyEmployee hourly = new HourlyEmployee(name, SSN, address, gender, ID, type);
                                int numberOfWorkingHours = Integer.parseInt(data[6]);
                                hourly.setNumbersOfHours(numberOfWorkingHours);
                                manager.saveEmployee(hourly);
                                break;
                            case Commission:
                                CommissionEmployee commission = new CommissionEmployee(name,SSN,address,gender,
                                        ID,type);
                                int sales = Integer.parseInt(data[6]);
                            ...
                            case Base_Plus_Commission:
                                BasePlusCommissionEmployee basePlusCommission = new BasePlusCommissionEmployee(name,
                                        SSN,address,gender,ID,type);
                                double mixSalary = Double.parseDouble(data[6]);
                                int mixSales = Integer.parseInt(data[7]);
                                double mixCommissionRate = Double.parseDouble(data[8]);
                                basePlusCommission.setSalary(mixSalary);
                                basePlusCommission.setGrossSales(mixSales);
                                basePlusCommission.setCommissionRate(mixCommissionRate);
                                manager.saveEmployee(basePlusCommission);
                                break;
                ...
                System.out.println("You have removed the old data.\n");
                clearFile(answer, FilePlace);
            }
        }
